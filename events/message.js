...
module.exports = async (bot, message) => {
    ...
    if (!bot.db.get('ignore_channel').find({ guild: message.guild.id, channel: message.channel.id }).value()) {
        if (!message.content.startsWith(bot.config.prefix)) {
            ...
            if (!bot.db.get('profil').find({ guild: message.guild.id, user: message.author.id }).value()) return bot.db.get('profil').push({ _id: Random.generateID(20), guild: message.guild.id, user: message.author.id, xp: xpWon, lvl: 0, but: 1500, money: 0, likes: 0, description: null, color: null, emote: null }).write();
